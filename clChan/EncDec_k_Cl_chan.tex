...
%Title          :   Encoding and Decoding across k uses of a Classical Channel
%Author         :   Vikesh Siddhu
%Email          :   vsiddhu@andrew.cmu.edu
%Description    :   Letters from an alphabet A are encoded via E into k joint 
%                   uses of a channel N and decoded back via D (k).
...
\begin{tikzpicture}[node distance=.75cm,auto,>=latex']
    \node [int] at (0,0) (a) {$\AC^{(k)}$};
    \node [below of = a] (aT) {Input};

    \node [chan1] at (2,0) (b) {$E^{(k)}$};

    \node [txBox] at (3.2,0) (c) {$k$};
    \node [chan2] at (4,2.5) (c1) {$N$};
    \node [chan2] at (4,1.5) (c3) {$N$};
    \node [chan2] at (4,-2.5) (c2) {$N$};
    
    \node [chan1] at (6,0) (d) {$D^{(k)}$};

    \node [int] at (8,0) (e) {$\AC^{(k)}$};
